      ...
      <div className="flex items-center gap-3">
        {cloudConnected && cloudCredits !== null && (
          <a href={cloudTopUpUrl} target="_blank" rel="noopener noreferrer"
            aria-label={`Cloud credits: ${cloudCredits.toFixed(2)}`}
        ...
        {(evmShort || solShort) && (
          <div className="wallet-wrapper relative inline-flex">
            <button onClick={() => setTab("inventory")} aria-label="View inventory" className="inline-flex items-center justify-center w-7 h-7 border border-border bg-bg cursor-pointer hover:border-accent hover:text-accent transition-colors">
