...
async function playAudio(data: Buffer, format: string): Promise<void> {
  ...
  try {
    if (os === "darwin") {
      ...
      const base64Path = Buffer.from(winPath).toString("base64");
      runCommand(
        "powershell",
        [
          "-Command",
          `$path = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String('${base64Path}')); (New-Object Media.SoundPlayer $path).PlaySync()`,
        ],
        60000,
      );
    }
