...
function readBody(req: http.IncomingMessage): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    req.on("data", (c: Buffer) => {
      totalBytes += c.length;
      if (totalBytes > 1_048_576) { req.destroy(); reject(new Error("Request body too large")); return; }
      chunks.push(c);
    });
...
export async function handleCloudRoute(
  ...
  if (method === "POST" && pathname === "/api/cloud/login") {
    ...
    if (!createRes.ok) { err(res, "Failed to create auth session with ELIZA Cloud", 502); return true; }
  ...
  if (method === "GET" && pathname.startsWith("/api/cloud/login/status")) {
    ...
    if (!sessionId) { err(res, "sessionId query parameter is required"); return true; }

    const baseUrl = state.config.cloud?.baseUrl ?? "https://www.elizacloud.ai";
    const pollRes = await fetch(`${baseUrl}/api/auth/cli-session/${encodeURIComponent(sessionId)}`);

    if (!pollRes.ok) {
      json(res, pollRes.status === 404
        ? { status: "expired", error: "Session not found or expired" }
        : { status: "error", error: `ELIZA Cloud returned HTTP ${pollRes.status}` });
      return true;
    }

    const data = (await pollRes.json()) as { status: string; apiKey?: string; keyPrefix?: string };

    if (data.status === "authenticated" && data.apiKey) {
      if (!state.config.cloud) (state.config as Record<string, unknown>).cloud = {};
      state.config.cloud!.enabled = true;
      state.config.cloud!.apiKey = data.apiKey;
      saveMilaidyConfig(state.config);

      if (state.cloudManager && !state.cloudManager.getClient()) state.cloudManager.init();
  ...
  if (method === "GET" && pathname === "/api/cloud/agents") {
    ...
    if (!client) { err(res, "Not connected to ELIZA Cloud", 401); return true; }
  ...
  if (method === "POST" && pathname === "/api/cloud/agents") {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) { err(res, "Request body must be a JSON object"); return true; }
    const body = parsed as { agentName?: string; agentConfig?: Record<string, unknown>; environmentVars?: Record<string, string> };
    if (!body.agentName?.trim()) { err(res, "agentName is required"); return true; }

    const agent = await client.createAgent({ agentName: body.agentName, agentConfig: body.agentConfig, environmentVars: body.environmentVars });
  ...
  if (method === "POST" && pathname.startsWith("/api/cloud/agents/") && pathname.endsWith("/provision")) {
    ...
    if (!agentId || !state.cloudManager) { err(res, "Invalid agent ID or cloud not connected", 400); return true; }
    const proxy = await state.cloudManager.connect(agentId);
    json(res, { ok: true, agentId, agentName: proxy.agentName, status: state.cloudManager.getStatus() });
    return true;
  }
